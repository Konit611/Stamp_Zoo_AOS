...
class LanguageStore(private val app: Application) {
    ...
    suspend fun setLanguage(tag: String, activity: ComponentActivity?) {
        if (BuildConfig.DEBUG) Log.d("LanguageStore", "Setting language to: $tag")

        // SharedPreferences에 먼저 저장 (동기, attachBaseContext용)
        val success = prefs.edit().putString("language_tag", tag).commit()
        if (BuildConfig.DEBUG) Log.d("LanguageStore", "SharedPreferences save success: $success")

        // DataStore에 저장 (비동기)
        app.langDataStore.edit { it[KEY_LANGUAGE] = tag }
        if (BuildConfig.DEBUG) Log.d("LanguageStore", "DataStore save completed")

        // Activity를 재생성하여 언어 변경 적용 (현재 화면 유지)
        if (activity != null) {
            if (BuildConfig.DEBUG) Log.d("LanguageStore", "Recreating activity")
            activity.recreate()
        } else {
            if (BuildConfig.DEBUG) Log.d("LanguageStore", "Activity is null, cannot recreate")
        }
    }
